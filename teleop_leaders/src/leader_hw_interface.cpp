...
LeaderHardwareInterface::LeaderHardwareInterface(ros::NodeHandle nh, ros::NodeHandle private_nh)
    : node_handle_(nh),
...
double LeaderHardwareInterface::CustomEncoder2Radian(int32_t encoderval)
{   
    double radianval = ((double)encoderval/4095.)*2.*M_PI;

    // Make inbetween -pi to pi
    radianval = fmod(radianval, 2.0*M_PI);
    if(radianval > M_PI)
        radianval -= 2.0*M_PI;
    else if(radianval < -M_PI)
        radianval += 2.0*M_PI;

    return radianval;
}

int32_t LeaderHardwareInterface::CustomRadian2Encoder(double radianval)
{   
    int32_t encoderval = (radianval/(2.*M_PI))*4095;
    return encoderval;
}

int32_t LeaderHardwareInterface::CustomTorqueToCurrent(double torque)
{
    double torque_constant = 0.0005; // NM/ma // some torque constant
    return (int32_t)(torque/torque_constant);
}

double LeaderHardwareInterface::CustomCurrentToTorque(int16_t current)
{
    double torque_constant = 0.0005; // NM/ma // some torque constant
    double current_d = (double)current;

    return torque_constant*current;
}

void LeaderHardwareInterface::registerActuatorInterfaces()
{
...
bool LeaderHardwareInterface::initWorkbench(const std::string port_name, const uint32_t baud_rate)
{
    bool result = false;
    const char *log;
...
bool LeaderHardwareInterface::getDynamixelsInfo(const std::string yaml_file)
{
    YAML::Node dynamixel;
...
bool LeaderHardwareInterface::loadDynamixels(void)
{
    bool result = false;
    const char *log;
    ...
    for (auto const &dxl : dynamixel_)
    {
        ...
        if(XC_model_numbers.find(model_number) != XC_model_numbers.end()){
            indaddr_info_[std::to_string(dxl.second)]["ADDR_INDADDR_READ"] = ADDR_INDADDR_READ_XC;
            indaddr_info_[std::to_string(dxl.second)]["ADDR_INDADDR_WRITE"] = ADDR_INDADDR_WRITE_XC;
        }
        else 
...
bool LeaderHardwareInterface::initControlItems(void)
{
    bool result = false;
    const char *log = NULL;
    ...
    for (auto const &dxl : dynamixel_)
    {
        const ControlItem *goal_current = dxl_wb_->getItemInfo(dxl.second, "Goal_Current");
        ...
        control_items_[std::to_string(dxl.second)]["Goal_Current"] = goal_current;
        ...
        // ROS_INFO_STREAM("goal_current->address: " << std::to_string(goal_current->address));
...
bool LeaderHardwareInterface::initDynamixels(void)
{
    const char *log;
    ...
    */
    for (auto const &dxl : dynamixel_)
    {
        ...
        {
            uint16_t data = control_items_[std::to_string(dxl.second)]["Goal_Current"]->address + j;
            uint8_t data_write[2] = { DXL_LOBYTE(data), DXL_HIBYTE(data) };
        ...
        // Torque On after setting up all servo  TODO
    // for (auto const &dxl : dynamixel_)
...
bool LeaderHardwareInterface::initSDKHandlers(void)
{  
    bool result = false;
    const char *log = NULL;
    ...
    result = dxl_wb_->addSyncWriteHandler(ADDR_INDDATA_WRITE_XC, 
                                            LEN_IND_WRITE, 
                                            &log);
    if (result == false)
    {
    ...
    result = dxl_wb_->addSyncReadHandler(ADDR_INDDATA_READ_XC,
                                            LEN_IND_READ,
                                            &log);
    if (result == false)
    {
...
void LeaderHardwareInterface::registerControlInterfaces()
{
    // resize vector
    ...
    for (auto iter = dynamixel_.begin(); iter != dynamixel_.end(); iter++)
    {
        ...
        ROS_INFO("joint_name : %s, servo ID: %d registered", iter->first.c_str(), iter->second);
...
void LeaderHardwareInterface::read()
{
    bool result = false;
    const char *log = NULL;
    ...
    result = dxl_wb_->getSyncReadData(sync_read_handler,
                                        id_array,
                                        id_cnt,
                                        ADDR_INDDATA_READ_XC + LEN_READ_POS,
                                        LEN_READ_VEL,
                                        get_velocity,
                                        &log);
    if (result == false)
    ...
    result = dxl_wb_->getSyncReadData(sync_read_handler,
                                        id_array,
                                        id_cnt,
                                        ADDR_INDDATA_READ_XC + LEN_READ_POS + LEN_READ_VEL,
                                        LEN_READ_CUR,
                                        get_current,
                                        &log);
    if (result == false)
    ...
    for (uint8_t index = 0; index < id_cnt; index++)
    {
        ...
        joints_[id_array[index] - 1].position = CustomEncoder2Radian((int32_t)get_position[index]);
        joints_[id_array[index] - 1].velocity = dxl_wb_->convertValue2Velocity((uint8_t)id_array[index], (int32_t)get_velocity[index]);
        joints_[id_array[index] - 1].effort = CustomCurrentToTorque(get_current[index]);

        joints_[id_array[index] - 1].effort_command = joints_[id_array[index] - 1].effort; // Some torque constant
    }
   
}

void LeaderHardwareInterface::write(const ros::Duration& loop_period)
{
    //TODO: Implement the write function if needed
    ...
    uint8_t id_cnt = 0;
    uint8_t sync_write_handler = 0; // 0 For non XC motors 1 for XC motors

    for (auto const &dxl : dynamixel_)
    {   
        id_array[id_cnt] = (uint8_t)dxl.second;
        dynamixel_current[id_cnt] = CustomTorqueToCurrent(joints_[dxl.second - 1].effort_command); // some torque constant
        // ROS_INFO_STREAM("motor id: " << std::to_string(id_array[id_cnt]));
        // ROS_INFO_STREAM("effort command: " << std::to_string(dynamixel_current[id_cnt]));
        id_cnt++;
    }


    if(id_cnt > 0){
        result = dxl_wb_->syncWrite(sync_write_handler, 
                                    id_array, 
                                    id_cnt,
                                    dynamixel_current,
                                    1,
