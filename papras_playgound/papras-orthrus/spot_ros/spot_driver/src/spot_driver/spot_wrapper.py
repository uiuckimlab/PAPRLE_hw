...
class SpotWrapper():
    ...
    def init_nav(self, upload_path):
        """Power on, stand robot up, initialize graph"""
    ...
    def _upload_graph_and_snapshots(self, upload_filepath):
        ...
        # Assume not localized -> always localize to fiducial
    ...
    def _navigate_to(self, *args):
        ...
        is_finished = False
        nav_to_cmd_id = -1
        linear_velocity = geometry_pb2.Vec2(x=0.5, y=0.5)
        max_velocity = geometry_pb2.SE2Velocity(linear=linear_velocity, angular=1.0) #m/s, rad/s
        vel_limit = geometry_pb2.SE2VelocityLimit(max_vel=max_velocity)
        travel_parameters = self._graph_nav_client.generate_travel_params(0.01, 0.05, vel_limit) #distance threshold, angle threshold
        while not is_finished:
            # Issue the navigation command about twice a second such that it is easy to terminate the
            # navigation command (with estop or killing the program).
            nav_to_cmd_id = self._graph_nav_client.navigate_to(destination_waypoint, 1.0,
                                                               travel_params=travel_parameters,
